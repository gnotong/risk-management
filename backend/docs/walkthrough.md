...
- **Keycloak Identity Management**: The client application is now tightly wrapped by the official `keycloak-js` adapter. Routing intercepts and forces a login prior to app mounting. The native browser `fetch` function was explicitly monkey-patched to transparently append an `Authorization: Bearer <keycloak.token>` header dynamically to all downstream API requests safely.
...
- **REST Method-Level RBAC**: Côté Backend, the 6 primary REST APIs (`RisqueResource`, `PlanActionResource`, `AuditResource`, etc.) are now intrinsically draped with explicit `jakarta.annotation.security.RolesAllowed("USER")` safeguards, strictly bounding structural payloads to formally authenticated and authorized accounts possessing this Keycloak realm role.
